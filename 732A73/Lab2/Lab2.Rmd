...
for (i in 2:100) {
  lines(data[,3], curve[,i], col = rgb(0,0,1,alpha=0.2))  # 淡蓝色
}
```
...
```{r eval=TRUE}
# define inverse chi square function
mu_0_new <- c(rep(2,3),rep(16,8))*0.02
nu_0<- 1
sig_0_sq <- 2
omega_0_new <- diag(c(rep(1, 3), rep(10, 8)))*10000 
...
plot(time[,2], temp, pch = 16, col = 'gray', xlab = "Time", ylab = "Temperature",ylim = range(curve_new[,i])
     main = "Posterior Median and 90% Prediction Interval")
for (i in 2:100) {
  lines(data[,3], curve_new[,i], col = rgb(0,0,1,alpha=0.2))  # 淡蓝色
}
```
