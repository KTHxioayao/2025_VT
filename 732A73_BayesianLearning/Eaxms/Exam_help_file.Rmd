...
### ? Prior Interpretation for the Posterior of $\sigma^2$
...
$$ where $$\tau^2 = \frac{1}{n} \sum_{i=1}^n (\log y_i - \mu)^2.$$
...
The posterior mode of $\lambda$ is defined as the value that maximizes the posterior distribution:

$$
\lambda_{\text{mode}} = \arg \max_{\lambda > 0} p(\lambda \mid y, \sigma).
$$

In Bayesian statistics, the posterior distribution $p(\lambda \mid y, \sigma)$ represents our updated belief about the parameter $\lambda$ after observing the data $y$. The mode of this distribution is the most probable value — that is, the value of $\lambda$ with the highest posterior density. This is also known as the maximum a posteriori (MAP) estimate.
...
for  (i in 1: 100) {
  ...
  # beta from sampeled sigma square 
...
We consider a Bayesian linear regression model with a conjugate prior. The response variable (e.g., temperature) is modeled as a quadratic function of time:
...
-   The prior for $$\ \boldsymbol{\beta} \mid \sigma^2 $$is:

$$
\boldsymbol{\beta} \mid \sigma^2 \sim \mathcal{N}(\boldsymbol{\mu}_0, \sigma^2 W_0^{-1})
$$
...
-   For $$\ \boldsymbol{\beta} \mid \sigma^2, \text{y} $$:

$$
\boldsymbol{\beta} \mid \sigma^2, \text{y} \sim \mathcal{N}(\boldsymbol{\mu}_n, \sigma^2 W_n^{-1})
$$

-   For $$ \sigma^2 \mid \text{y} $$:

$$
\sigma^2 \mid \text{y} \sim \text{Inv-}\chi^2(\nu_n, \sigma_0^2)
$$
...
$$
\boldsymbol{\mu}_n = W_n^{-1} (W_0 \boldsymbol{\mu}_0 + X^\top y)
$$ with $y = X \boldsymbol{\beta} + \varepsilon$, the standard linear regression form. In calculation just use $y = X \boldsymbol{\beta}$.
...
$$
\nu_n \sigma_0^2 = \nu_0 \sigma_0^2 + (\boldsymbol{\mu}_0^\top W_0 \boldsymbol{\mu}_0 + y^\top y - \boldsymbol{\mu}_n^\top W_n \boldsymbol{\mu}_n)
$$
...
#### ?Derivation of the Log-Posterior in Logistic Regression 

We begin by specifying the probability model used in logistic regression. The likelihood for each observation is given by the logistic (sigmoid) function:

$$
P(y_i = 1 \mid \boldsymbol{x}_i, \boldsymbol{\beta}) = \frac{1}{1 + \exp(-\boldsymbol{x}_i^\top \boldsymbol{\beta})}.
$$

The Bernoulli likelihood for a single observation $$ y_i \in \{0, 1\} $$ is:

$$
p(y_i \mid \boldsymbol{x}_i, \boldsymbol{\beta}) = \left( \frac{1}{1 + \exp(-\boldsymbol{x}_i^\top \boldsymbol{\beta})} \right)^{y_i}
\left( 1 - \frac{1}{1 + \exp(-\boldsymbol{x}_i^\top \boldsymbol{\beta})} \right)^{1 - y_i}.
$$
...
$$
p(y_i \mid \boldsymbol{x}_i, \boldsymbol{\beta}) = \left( \frac{\exp(\boldsymbol{x}_i^\top \boldsymbol{\beta})}{1 + \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta})} \right)^{y_i}
\left( \frac{1}{1 + \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta})} \right)^{1 - y_i}.
$$

The likelihood for the full dataset with \( n \) observations is:

$$
p(\boldsymbol{y} \mid \boldsymbol{\beta}, X) = \prod_{i=1}^n \left( \frac{\exp(\boldsymbol{x}_i^\top \boldsymbol{\beta})}{1 + \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta})} \right)^{y_i}
\left( \frac{1}{1 + \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta})} \right)^{1 - y_i}.
$$
...
$$
\log p(\boldsymbol{y} \mid \boldsymbol{\beta}, X) = \sum_{i=1}^n \left[ y_i (\boldsymbol{x}_i^\top \boldsymbol{\beta}) - \log(1 + \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta})) \right].
$$
...
Finally, if a prior distribution is placed on $$ \boldsymbol{\beta} $$, such as a Gaussian prior $$ \boldsymbol{\beta} \sim \mathcal{N}(0, \tau^2 I) $$, then the **log-posterior** becomes:

$$
\log p(\boldsymbol{\beta} \mid \boldsymbol{y}, X) \propto \log p(\boldsymbol{y} \mid \boldsymbol{\beta}, X) - \frac{1}{2 \tau^2} \boldsymbol{\beta}^\top \boldsymbol{\beta}.
$$

This is the target function optimized numerically to obtain the posterior mode $$ \tilde{\boldsymbol{\beta}} $$.


#### ?Bayesian Logistic Regression: Log-Posterior Formulation

In Bayesian logistic regression, the probability of a binary outcome is modeled using the logistic function:

$$
P(y_i = 1 \mid \boldsymbol{x}_i, \boldsymbol{\beta}) = \frac{1}{1 + \exp(-\boldsymbol{x}_i^\top \boldsymbol{\beta})}.
$$

Given observed outcomes $$y_i \in \{0, 1\}$$, the log-likelihood for all $$ n $$ observations is:
...
NOTICE: 
log_lik <- sum(y * lin_pred - log1p(exp(lin_pred))),其中 log1p(x) 是 $\log(1 + x)$ 的稳定实现，用于防止数值溢出。
...
$$
\log p(\boldsymbol{\beta} \mid \boldsymbol{y}, X) \propto \log p(\boldsymbol{y} \mid \boldsymbol{\beta}, X) + \log p(\boldsymbol{\beta}).
$$

The posterior distribution of $$ \boldsymbol{\beta} $$ is typically not available in closed form for logistic regression due to the non-conjugate likelihood. However, we can approximate the posterior using a second-order Taylor expansion of the log-posterior around its mode.

Starting from the log-posterior expression:

$$
\log p(\boldsymbol{\beta} \mid \boldsymbol{y}, X) \propto \log p(\boldsymbol{y} \mid \boldsymbol{\beta}, X) + \log p(\boldsymbol{\beta}),
$$

we define $$ \tilde{\boldsymbol{\beta}} $$ as the value of $$ \boldsymbol{\beta} $$ that maximizes this log-posterior, i.e., the posterior mode.

Using a Laplace approximation, we approximate the posterior distribution with a Gaussian centered at the mode $$ \tilde{\boldsymbol{\beta}} $$ and covariance given by the inverse of the negative Hessian of the log-posterior evaluated at $$ \tilde{\boldsymbol{\beta}} $$. This gives:

$$
\boldsymbol{\beta} \mid \boldsymbol{y}, X \approx \mathcal{N}(\tilde{\boldsymbol{\beta}}, J_y^{-1}(\tilde{\boldsymbol{\beta}})),
$$

where $$ J_y(\tilde{\boldsymbol{\beta}}) $$ is the observed information matrix:

$$
J_y(\tilde{\boldsymbol{\beta}}) = - \nabla^2 \log p(\boldsymbol{\beta} \mid \boldsymbol{y}, X) \big|_{\boldsymbol{\beta} = \tilde{\boldsymbol{\beta}}}.
$$
...
#When you plot a histogram with prob = TRUE, or when you use density(), the 
#y-axis represents density, not probability. That means the height of the curve 
#can exceed 1, as long as the total area under the curve is 1.
```

#### ? Gibbs Sampling for Bayesian Logistic Regression using Polya-Gamma Augmentation

In Bayesian logistic regression, direct sampling from the posterior distribution of the coefficients $\boldsymbol{\beta}$ is challenging due to the non-conjugate likelihood. One efficient approach to address this is **Polya-Gamma data augmentation**, which introduces latent variables to make the conditional distributions tractable for Gibbs sampling.

We assume a prior for the regression coefficients:

$$
\boldsymbol{\beta} \sim \mathcal{N}(\boldsymbol{b}, B),
$$

and introduce latent variables $w = (w_1, \ldots, w_n)$ such that the joint posterior becomes:

$$
p(\boldsymbol{\beta}, \boldsymbol{w} \mid \boldsymbol{y}) \propto p(\boldsymbol{y} \mid \boldsymbol{\beta}, \boldsymbol{w}) \, p(\boldsymbol{w} \mid \boldsymbol{\beta}) \, p(\boldsymbol{\beta}).
$$

In each iteration of the Gibbs sampler:

1.  For $i = 1, \ldots, n$, sample each latent variable: $$
    w_i \mid \boldsymbol{\beta} \sim \text{PG}(1, \boldsymbol{x}_i^\top \boldsymbol{\beta}),
...
2.  Then sample the regression coefficients from the conditional Gaussian posterior: $$
    \boldsymbol{\beta} \mid \boldsymbol{y}, \boldsymbol{w} \sim \mathcal{N}(\boldsymbol{m}_w, V_w),
    $$ where $$
    V_w = \left(X^\top W X + B^{-1} \right)^{-1}, \quad \boldsymbol{m}_w = V_w \left(X^\top \boldsymbol{\kappa} + B^{-1} \boldsymbol{b} \right),
    $$ with $W = \text{diag}(w_1, \ldots, w_n)$ and $\kappa_i = y_i - \frac{1}{2}$.
...
In Markov Chain Monte Carlo (MCMC), the samples $\{q^{(1)}, q^{(2)}, \ldots, q^{(N)}\}$ are usually autocorrelated, which means that the variance of the sample mean $\bar{q}$ does not decrease as fast as it would with independent samples.
...
where $\sigma^2$ is the marginal variance of $q^{(i)}$, and $\text{IF}$ is the **inefficiency factor**, also called the **integrated autocorrelation time**.
...
where $\rho_k$ is the lag-$k$ autocorrelation of the MCMC samples. This expression shows how autocorrelation inflates the variance of the estimator. When samples are independent, all $\rho_k = 0$, and $\text{IF} = 1$.

In practice, the infinite sum is truncated at some lag $K$ where $\rho_k$ becomes negligible, giving the estimate:

$$
\widehat{\text{IF}} = 1 + 2 \sum_{k=1}^{K} \widehat{\rho}_k.
$$
...
summary(model)
```
If p < 0.05, the variable is generally considered to have a statistically significant effect.
As is shown, significant covariants are VerifyID,  MinBidShare, Sealed, MajBlem, intercept and MinBidShare.

------------------------------------------------------------------------

#### ?Derivation of the Log-Posterior in Poisson Regression

We model the response variable $$ y_i\in \mathbb{N} $$ using a Poisson distribution:

$$
y_i \mid \boldsymbol{x}_i, \boldsymbol{\beta} \sim \text{Poisson}(\lambda_i), \quad \text{where } \lambda_i = \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta}).
$$
...
Substituting $$ \lambda_i = \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta}) $$, we obtain:

$$
p(y_i \mid \boldsymbol{x}_i, \boldsymbol{\beta}) = \frac{\left( \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta}) \right)^{y_i} \exp\left(-\exp(\boldsymbol{x}_i^\top \boldsymbol{\beta})\right)}{y_i!}.
$$

The **log-likelihood** for a single observation becomes:

$$
\log p(y_i \mid \boldsymbol{x}_i, \boldsymbol{\beta}) = y_i (\boldsymbol{x}_i^\top \boldsymbol{\beta}) - \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta}) - \log(y_i!).
$$

Summing over all n  observations, the total log-likelihood is:

$$
\log p(\boldsymbol{y} \mid \boldsymbol{\beta}, X) = \sum_{i=1}^n \left[ y_i (\boldsymbol{x}_i^\top \boldsymbol{\beta}) - \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta}) - \log(y_i!) \right].
$$

Since $$ \log(y_i!) $$ does not depend on $$ \boldsymbol{\beta} $$, it can be omitted in optimization or posterior sampling. Thus, we use the simplified log-likelihood:

$$
\log p(\boldsymbol{y} \mid \boldsymbol{\beta}, X) \propto \sum_{i=1}^n \left[ y_i (\boldsymbol{x}_i^\top \boldsymbol{\beta}) - \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta}) \right].
$$

 Prior

We place a Gaussian prior on $$ \boldsymbol{\beta} $$:

$$
\boldsymbol{\beta} \sim \mathcal{N}(0, g (X^\top X)^{-1}),
$$

where  g = 100  is a scalar. The log prior is:

$$
\log p(\boldsymbol{\beta}) = -\frac{1}{2} \boldsymbol{\beta}^\top \left( \frac{1}{g} X^\top X \right) \boldsymbol{\beta} + \text{const}.
$$
...
$$
\log p(\boldsymbol{\beta} \mid \boldsymbol{y}, X) \propto \sum_{i=1}^n \left[ y_i (\boldsymbol{x}_i^\top \boldsymbol{\beta}) - \exp(\boldsymbol{x}_i^\top \boldsymbol{\beta}) \right] - \frac{1}{2} \boldsymbol{\beta}^\top \left( \frac{1}{g} X^\top X \right) \boldsymbol{\beta}.
$$
...
LogPostPoisson <- function(beta,y,X){
  ...
  #added - lgamma(y + 1) term, which represents -log(y!) in the Poisson log-likelihood
  # lgamma(y + 1) correctly computes log(𝑦!)log(y!) and 
  #handles non-integer/floating-point y safely.
  log_lik <- sum(y * lin_pred - exp(lin_pred) - lgamma(y + 1))
...
cat("\nPosterior covariance matrix (approx.):\n")
print(cov_matrix)
```
...
$$
 $P(y_i = 1) = \sigma(\boldsymbol{x}_i^\top \boldsymbol{\beta})$
...
```{r eval=TRUE}
set.seed(123)
possion_samp <- mp_sim(1,LogPostFunc = LogPostPoisson, 
                       ndraws=10000, 
