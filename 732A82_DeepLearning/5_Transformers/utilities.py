...
def scaled_dot_product_attention(q, k, v):
    ...
    scaled_attention_logits = matmul_qk / tf.math.sqrt(dk)  # Shape: (batch_size, num_heads, seq_len_q, seq_len_k)
...
class MultiHeadAttention(layers.Layer):
    def __init__(self, n_heads, proj_size, dk, dv, return_attention_weights=False):
        ...
        self.WQ = tf.Variable(    
            initializer(shape=(n_heads, dk, proj_size)), 
            trainable=True)  # of shape (n_heads, dq=dk, proj_size)
        self.WK = tf.Variable(
            initializer(shape=(n_heads, dk, proj_size)), 
            trainable=True)  # of shape (n_heads, dk=dk, proj_size,)
        self.WV = tf.Variable(
            initializer(shape=(n_heads, dv, proj_size)), 
            trainable=True)  # of shape (n_heads, dv, proj_size,)
        self.WO = tf.Variable(
            initializer(shape=(n_heads*proj_size, proj_size)), 
            trainable=True)  # of shape (n_heads*proj_size, proj_size)
    ...
    def call(self, Q, K, V):
        ...
        A = tf.reshape(
            attention_pooling_h,
            shape=(tf.shape(attention_pooling_h)[0],  # batch_size
            tf.shape(attention_pooling_h)[2],  # number_of_Q
            tf.shape(attention_pooling_h)[1] * tf.shape(attention_pooling_h)[3])  # n_heads * proj_size
                    )  # of shape (batch_size, number_of_Q, n_heads*proj_dim)
...
def mlp(x, hidden_units, dropout_rate):
    ...
    for units in hidden_units:
        # Apply dense layer with GELU activation
...
def transformerBlock(x, num_heads, projection_dim, transformer_units, dropout_rate=0.1):
    ...
    attention_output = MultiHeadAttention(
        ...
        dk=projection_dim,
        dv=projection_dim
    )(x, x, x) # NOTE: here we are using the MultiHeadAttention layer to compute the SELF ATTENTION
...
class PatchEncoder(Layer):
    ...
    def call(self, patch):
        ...
        class_token = tf.tile(
            tf.expand_dims(self.class_token, 0),
            [batch, 1, 1]                     # Tile along batch dimension
        )
...
def create_vit_classifier(
    ...
    # Create patches.
    patches = PatchExtractor(patch_size)(inputs)  # (batch_size, num_patches, patch_dims)
    ...
    encoded_patches = PatchEncoder(num_patches, embedding_proj_dim)(patches)  # (batch_size, num_patches, projection_dim)
    ...
    representation = encoded_patches[:, 0]  # (batch_size, projection_dim)
