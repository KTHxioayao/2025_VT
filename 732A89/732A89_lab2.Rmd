...
```
Whether a point is a saddle point or local minimum/ maximum can be told from the eign values of Hessian matrix:
  If all the eigenvalues of the Hessian matrix are positive, the critical point is a local minimum.
  
  If all the eigenvalues are negative, the critical point is a local maximum.
  
  If the Hessian matrix has both positive and negative eigenvalues, the critical point is a saddle point.
  
The last solution gives (-0.55, -1,54) and (2.59, 1.59)  the local minimum. (1.54, 0.55) is a saddle point. 
Since the function has a lower value in (-0.55, -1,54), it is a also global minimum.
```{r echo=FALSE}
...
```
As is shown, the original function has the fastest convergence speed at the 38 th iteration. When α is defined as min((i+1)/999, it has a quite small step and takes more times to convergence. When α*sqrt(i+5) is applied, the step decreases with the increasing of iteration number, which gives a quicker convergence. 
...
We get the similar results for the BFGS and the Nelder-Mead algorithm and they are also very close to b. The number of function and gradient evaluations are different for the BFGS and the Nelder-Mead algorithm. The BFGS method gives 12 functions and 8 gradient and Nelder-Mead gives 47 functions and NA gradiet, it seems that Nelder-Mead algorithm need more iterations to converge because lacking of the gradient information.
...
The result is similar to the result from the steepest ascent method. but the iteration number is different. the glm function is only 4, the steepest ascent need 38 times.
